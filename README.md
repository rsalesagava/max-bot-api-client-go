# Max Bot API Client library for Golang


## Документация

В [документации](https://github.com/max-messenger/max-bot-api-client-go/tree/master/docs) вы можете найти подробные инструкции по использованию фреймворка.

## Быстрый старт

> Если вы новичок, то можете прочитать [официальную документацию](https://dev.max.ru/), написанную разработчиками Max

### Получение токена
Откройте диалог с [MasterBot](https://max.ru/MasterBot), следуйте инструкциям и создайте нового бота. После создания бота MasterBot отправит вам токен.

## Примеры

- [example/simple](example/simple) — демонстрация основных возможностей API: отправка текста, вложений, клавиатур, обработка callback-кнопок.
- [example/subscription-bot](example/subscription-bot) — готовый бот проверки подписки на канал: выдаёт ссылку на подборку товаров после подписки, пользователей записывает в текстовый файл (без БД).
